public class Pet {
    ...
    // SAMPLE ACTIONS
    public void play(){
        setFun(fun + 0.3); // implement if-else depending on how long the user play the mini-games
    ...
    public void feed(){
        setHunger(hunger + 0.3); // implement if -else depending on the food
        setCleanliness(cleanliness - 0.02);
        setFun(fun + 0.1);
    }

    public void bath(){
        setCleanliness(cleanliness + 0.05); // implement if-else 
        setSleep(sleep + 0.05);
    }
